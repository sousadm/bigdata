...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('etl_clientes.log'),
        logging.StreamHandler(sys.stdout)
    ]
)

class SQLServerConnector:
    def __init__(self, server="localhost", database="ASERP", user="sa", password="Admin.Server.2025"):
...
class ETLClientes:
    ...
    def create_table(self):
        ...
        try:
            ...
            logging.info("Tabela 'dim_cliente' criada/verificada")
        except Exception as e:
    ...
    def extract_batch(self, batch_size=None):
        ...
        try:
            count_query = "SELECT COUNT(*) AS total FROM cliente"
            ...
            while offset < total_rows:
                query = f"""
                SELECT id_cliente, cpf_cnpj, nome_fantasia, razao_social
                FROM cliente
                ORDER BY id_cliente
                OFFSET {offset} ROWS
    ...
    def extract(self):
        """Método legado (não usado no modo batch)"""
        # CORREÇÃO: Removendo o bloco 'except' inválido
        return None 
        # Linhas 113-115 originais removidas.
    ...
    def load(self, df):
        try:
            ...
            self.ch.client.execute(
                "INSERT INTO dim_cliente (id_cliente, cpf_cnpj, nome_fantasia, razao_social) VALUES",
                data
            )
    ...
    def verify(self):
        try:
            total = self.ch.client.execute("SELECT COUNT(*) FROM dim_cliente")[0][0]
    ...
    def run(self):
        ...
        try:
            ...
            self.sql.disconnect()
            self.ch.disconnect()
            # CORREÇÃO: O bloco 'finally' duplicado foi removido daqui.
