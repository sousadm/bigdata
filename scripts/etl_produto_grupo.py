...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('etl_produto_grupo.log'), 
...
class ETLProdutos: 
    ...
    def create_table(self):
        query = """
            CREATE TABLE IF NOT EXISTS default.dim_produto_grupo
            (
                `id` UInt32,
                `descricao` String,
        ...
        try:
            ...
            logging.info("Tabela 'dim_produto_grupo' criada/verificada")
        except Exception as e:
    ...
    def extract_batch(self, batch_size=None):
        ...
        try:
            count_query = """
                ...
                FROM ASPROD.dbo.Grupos
            """

            total_rows = pd.read_sql(count_query, self.sql.conn).iloc[0]["total"]
            logging.info(f"Total de registros (Grupos): {total_rows}")

            offset = 0
            while offset < total_rows:
                # Usando ROW_NUMBER() para compatibilidade com SQL Server antigo
                query = f"""
                SELECT id, descricao
                FROM (
                    SELECT 
                        Grupo AS id,
                        Nome AS descricao,
                        ROW_NUMBER() OVER (ORDER BY Grupo) AS RowNum
                    FROM ASPROD.dbo.Grupos
                ) AS Subquery
    ...
    def load(self, df):
        try:
            ...
            self.ch.client.execute(
                "INSERT INTO dim_produto_grupo (id, descricao) VALUES",
                data
            )
    ...
    def verify(self):
        try:
            total = self.ch.client.execute("SELECT COUNT(*) FROM dim_produto_grupo")[0][0]
